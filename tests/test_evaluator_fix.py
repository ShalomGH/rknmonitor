...
async def test_evaluate_all_targets_batched():
    """evaluate_targets fetches latest results in 3 flat queries, not N+1."""
    target_rows = [
        {"id": 1, "domain": "example.com"},
        {"id": 2, "domain": "blocked.ru"},
    ]
    # result is JSONB → asyncpg returns dict, not str
    http_rows = [
        {"target_id": 1, "result": {"status_code": 200}},
        {"target_id": 2, "result": {"status_code": None, "error": "timeout"}},
    ]
    dns_rows = [
        {"target_id": 1, "result": {"results": [{"ip": "93.184.216.34"}]}},
    ]
    ...
    with patch("rknmon.probes.evaluator.fetch", new_callable=AsyncMock) as mock_fetch, \
        ...
        mock_fetch.side_effect = [target_rows, http_rows, dns_rows]
        mock_state.return_value = None

        await evaluate_targets(target_ids=None)

        # Should be exactly 3 fetches: targets, http, dns
        assert mock_fetch.call_count == 3
        calls = mock_fetch.call_args_list
        assert "targets" in calls[0][0][0].lower()
        assert "probe_type = 'http'" in calls[1][0][0]
        assert "probe_type = 'dns'" in calls[2][0][0]

        # Should call update_target_state for each target
...
async def test_evaluate_targets_filter_by_ids():
    ...
    with patch("rknmon.probes.evaluator.fetch", new_callable=AsyncMock) as mock_fetch, \
        ...
        mock_fetch.side_effect = [[{"id": 5, "domain": "a.com"}], [], []]
        mock_state.return_value = None
