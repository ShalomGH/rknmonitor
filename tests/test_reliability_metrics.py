...
async def test_subject_failure_is_blocked_when_fresh_control_succeeds():
    ...
    http_rows = [
        {"target_id": 1, "probe_node_id": 7, "result": {"reachable": False, "error": "timeout"}, "checked_at": now},
        {"target_id": 1, "probe_node_id": 9, "result": {"reachable": True, "status_code": 200}, "checked_at": now - timedelta(seconds=5)},
    ]
    dns_rows = [
...
async def test_stale_control_result_does_not_confirm_blocking():
    ...
    http_rows = [
        {"target_id": 1, "probe_node_id": 7, "result": {"reachable": False, "error": "timeout"}, "checked_at": now},
        {"target_id": 1, "probe_node_id": 9, "result": {"reachable": True, "status_code": 200}, "checked_at": now - timedelta(hours=1)},
    ]
    dns_rows = [
        {"target_id": 1, "probe_node_id": 7, "result": {"tampered": False, "results": []}},
        {"target_id": 1, "probe_node_id": 9, "result": {"tampered": False, "results": []}},
...
async def test_global_state_query_prefers_subject_roles():
    ...
    mock_update.assert_called_once_with({"clear": 1}, {"blocked": 1})
