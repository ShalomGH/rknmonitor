...
def _timestamps_match(
    ...
    *,
    ...
    comparison_ts = comparison_at if comparison_at.tzinfo else comparison_at.replace(tzinfo=timezone.utc)
...
async def evaluate_targets(target_ids: List[int] | None = None) -> List[dict]:
    ...
    node_roles = {int(r["probe_node_id"]): str(r.get("role") or "subject") for r in node_rows}
    node_names = {int(r["probe_node_id"]): str(r.get("name") or r["probe_node_id"]) for r in node_rows}
    ...
    http_map = {
        (int(r["target_id"]), int(r["probe_node_id"])): (_decode_jsonb(r["result"]), r.get("checked_at"))
        for r in http_rows
    }
    dns_map = {
            ...
            if role == "subject" and http_result is not None:
                ...
                for comparison_node_id in probe_node_ids:
                    ...
                    if comparison_role not in _COMPARISON_ROLES:
                        continue
                    comparison_result, comparison_at = http_map.get((tid, comparison_node_id), (None, None))
